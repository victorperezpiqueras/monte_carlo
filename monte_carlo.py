...
class MonteCarloSimulation:
    ...
    def get_all_issues_data(self):
        ...
        last_year_start = (now - datetime.timedelta(days=365)).strftime("%Y-%m-%d")
    ...
    def sample_distribution(self, issues, sample_size=10000):
        ...
        plt.plot(x, y, color='black', linestyle='solid', label='Probability Density Function of real data')
        ...
        print(f"SAMPLES: Mean : {sampled_mean} and Standard deviation : {sampled_std}")
        # Calculate statistics for the samples
        t_stat, p_value = stats.ttest_ind(cycle_times_days, samples)
        print(f"t-statistic: {t_stat}")
        print(f"p-value: {p_value}")
        plt.hist(samples, bins=60, density=True, rwidth=0.9, label=f'Estimated from samples with p-value = {p_value}')
    ...
    def get_story_points_relationship(self, issues):
        plt.figure(figsize=(14, 8))
        story_points = [issue["fields"]["story_points"] for issue in issues if issue["fields"]["story_points"]]
        cycle_times = [issue["fields"]["cycle_time"].total_seconds() for issue in issues if
                       issue["fields"]["cycle_time"]]
        cycle_times_days = [cycle_times_day / 86400 for cycle_times_day in cycle_times]
